// src/components/ExposicaoSection.tsx
import { CalendarDays, MapPin, Clock, FileText, Award, Download, Image as ImageIcon } from "lucide-react";
import { EXPOSICOES_DADOS } from "@/data/exposicoesData";

interface ExposicaoSectionProps {
  ano: string; // Ex: "2026"
}

export default function ExposicaoSection({ ano }: ExposicaoSectionProps) {
  const info = EXPOSICOES_DADOS[ano] || EXPOSICOES_DADOS["2026"];

  return (
    <section id={`exposicao-${ano}`} className="py-24 bg-background text-foreground relative overflow-hidden border-t border-border/40">
      <div className="container-page max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center mb-16">
          ...
          <p className="text-muted-foreground text-base sm:text-lg">
            Consulte toda a informação oficial, regulamentos, prémios, inscrições e galeria da exposição de {info.ano}.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm flex flex-col justify-between">
            <div>
              <div className="h-12 w-12 rounded-xl bg-primary/10 flex items-center justify-center text-primary mb-4">
                <CalendarDays className="h-6 o-6" />
              </div>
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Condições de admissão e escalões de {info.ano}.
              </p>
            </div>
            <a href={info.regulamentoUrl} className="inline-flex items-center justify-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground shadow transition hover:bg-primary/90 mt-4">
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-6 shadow-sm flex flex-col justify-between">
            ...
            <a href={info.inscricoesUrl} className="inline-flex items-center justify-center gap-2 rounded-xl border border-border bg-background px-4 py-2.5 text-sm font-medium shadow-sm transition hover:bg-muted mt-4">
